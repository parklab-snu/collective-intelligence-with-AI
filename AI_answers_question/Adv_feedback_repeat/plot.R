...
save_path <- "C:/Users/glaucous_winged_gull/Desktop/2026_Park_lab/Collective-intelligence-with-AI/AI_answers_question/Adv_feedback_repeat2"
...
df <- do.call(rbind, lapply(seq_len(nrow(grid)), function(idx) {
  ...
  fp <- file.path(save_path, sprintf("adv_feedback_k%02d_i%03d_j%02f.RData", replicate, i, j))
...
stationary_rep <- df %>%
  filter(Generation >= 200000, Generation <= 200000) %>%
...
ggsave(
  file.path(save_path, "Adv_niche_Stationary_vs_Lambda_accuracy_belief_by_bias_16.png"),
  p_combined,
  width = 10,
  height = 4,
  bg = "white"
